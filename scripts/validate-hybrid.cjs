...
function findMatches(content) {
  ...
  // Regex that matches only true src attribute (excludes data-hybrid-src)
  const SCRIPT_WITH_SRC = /<script[^>]*?(?<![\w-])src\s*=\s*"[^"]+"[^>]*>/gi;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const scriptSrcMatches = line.match(SCRIPT_WITH_SRC) || [];
    ...
    for (const tag of scriptSrcMatches) {
      ...
      if (/\bsrc\s*=\s*"\s*\{\{[^}]*\|\s*asset_url/i.test(tag)) {
